...
// 线
class EdgeAnalyticCostFunction : public ceres::SizedCostFunction<3, 4, 3> {
public:
    EdgeAnalyticCostFunction(Eigen::Vector3d curr_point_, Eigen::Vector3d last_point_a_,
                        ...
                        double** jacobians) const 
	{
		...
		if(jacobians != NULL)
		{
			if(jacobians[0] != NULL)
			{
				...
				dp_by_dr.block<3,3>(0,0) = -skew_lp;
				Eigen::Map<Eigen::Matrix<double, 3, 4, Eigen::RowMajor> > J_se3_r(jacobians[0]);
				J_se3_r.setZero();
				J_se3_r.block<3,3>(0,0) = skew_re * dp_by_dr / de.norm();

				//t
				Eigen::Matrix<double, 3, 3> dp_by_dt;
				(dp_by_dt.block<3,3>(0,0)).setIdentity();
				Eigen::Map<Eigen::Matrix<double, 3, 3, Eigen::RowMajor> > J_se3_t(jacobians[1]);
				J_se3_t.setZero();
				J_se3_t.block<3,3>(0,0) = skew_re * dp_by_dt / de.norm();	
			}
		}
                        ...
                        double** jacobians) const 
	{
		...
		if(jacobians != NULL)
		{
			if(jacobians[0] != NULL)
			{
				...
				dp_dr.block<3,3>(0,0) = -skew_lp;
				Eigen::Map<Eigen::Matrix<double, 1, 4, Eigen::RowMajor> > J_se3_r(jacobians[0]);
				J_se3_r.setZero();
				J_se3_r.block<1,3>(0,0) = dX_dp.transpose() * dp_dr;

				// t
				Eigen::Matrix<double, 3, 3> dp_dt;
				(dp_dt.block<3,3>(0,0)).setIdentity();
				Eigen::Map<Eigen::Matrix<double, 1, 3, Eigen::RowMajor> > J_se3_t(jacobians[1]);
				J_se3_t.setZero();
				J_se3_t.block<1,3>(0,0) = dX_dp.transpose() * dp_dt;
			}
		}
