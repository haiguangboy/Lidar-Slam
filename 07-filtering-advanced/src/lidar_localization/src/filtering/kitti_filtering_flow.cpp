...
bool KITTIFilteringFlow::SaveOdometry(void) {
    ...
    std::ofstream fused_vel_ofs;
    std::ofstream ref_vel_ofs;
    if (
        ...
        !FileManager::CreateFile(fused_vel_ofs, WORK_SPACE_PATH + "/slam_data/trajectory/vel_fused.txt")||
        !FileManager::CreateFile(ref_vel_ofs, WORK_SPACE_PATH + "/slam_data/trajectory/vel_ground_truth.txt")
    ) {
        return false;
    }
    ...
    for (size_t i = 0; i < trajectory.N; ++i) {
        ...
        const Eigen::Matrix3f &Cnb = current_gnss_data_.pose.block<3, 3>(0, 0);

        SaveVel(velocity_n.time_.at(i), Cnb.transpose()*velocity_n.fused_.at(i), fused_vel_ofs);
        SaveVel(velocity_n.time_.at(i), current_gnss_data_.vel, ref_vel_ofs);
    }
...
bool KITTIFilteringFlow::UpdateOdometry(const double &time) {
    ...
    velocity_n.time_.push_back(time);
    
    velocity_n.fused_.push_back(fused_vel_);
