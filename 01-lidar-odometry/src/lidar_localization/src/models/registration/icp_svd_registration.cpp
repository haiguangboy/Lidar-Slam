...
void ICPSVDRegistration::GetTransform(
    ...
    int  N = xs.size();
    Eigen::Vector3f  xc=Eigen::Vector3f::Zero();
    Eigen::Vector3f  yc=Eigen::Vector3f::Zero();
    for (int i=0; i<N; i++){
        xc[0] += xs[i][0];
        xc[1] += xs[i][1];
        xc[2] += xs[i][2];
    ...
    yc[2] = yc[2]/N;
/*
    Eigen::Vector3f  xd=Eigen::Vector3f::Zero();
    Eigen::Vector3f  yd=Eigen::Vector3f::Zero();
    ...
    for (int i=0; i<N; i++){
        H =H + ( ys[i]- yc) * (xs[i] - xc).transpose();
    }

    // TODO -- solve R:
       Eigen::Matrix3f U, V;
       Eigen::JacobiSVD<Eigen::Matrix3f> svd_solver(H, Eigen::ComputeFullU | Eigen::ComputeFullV);
       U = svd_solver.matrixU();
       V = svd_solver.matrixV();

      Eigen::Matrix3f  R =  Eigen::Matrix3f ::Zero(3,3);
      Eigen::Vector3f  t =  Eigen::Vector3f ::Zero(3,1);
